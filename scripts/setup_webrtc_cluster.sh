...
CONFIG_FILE="scripts/kind-cluster-config-webrtc-cluster.yaml"

echo "==> [1/6] Cleaning up existing KinD cluster if present..."
kind delete cluster --name "$CLUSTER_NAME" || true

echo "==> [2/6] Creating WebRTC KinD Cluster using configuration file..."
...
# 2. Install Kubernetes Gateway API CRDs (Required for Waypoint proxies)
kubectl apply --server-side -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.2.0/standard-install.yaml

# 3. Install Istio CNI (chains into Cilium for traffic redirection)
...
echo "==> [5/6] Applying Namespaces and Custom Manifests via Kustomize..."
kubectl create namespace business-app --dry-run=client -o yaml | kubectl apply -f -
kubectl create namespace analytics-tier --dry-run=client -o yaml | kubectl apply -f -

if [ -d "manifests/base/apps" ]; then
  kubectl apply -k manifests/base/apps/
fi
